...
AC_ARG_WITH([imlib-prefix],
  AS_HELP_STRING([--with-imlib-prefix=PFX], [Prefix where IMLIB is installed (optional)]),
[imlib_prefix="$withval"], [imlib_prefix=""])

AC_ARG_WITH([imlib-exec-prefix],
  AS_HELP_STRING([--with-imlib-exec-prefix=PFX], [Exec prefix where IMLIB is installed (optional)]),
[imlib_exec_prefix="$withval"], [imlib_exec_prefix=""])

AC_ARG_ENABLE([imlibtest],
  AS_HELP_STRING([--disable-imlibtest], [Do not try to compile and run a test IMLIB program]),
