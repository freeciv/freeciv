    ...
    if (give) {
      if (ds_after == DS_ALLIANCE) {
        if (!shared_vision_is_safe(pplayer, aplayer)) {
          dai_diplo_notify(aplayer, _("*%s (AI)* Sorry, sharing vision with you "
                                      "is not safe."),
                           player_name(pplayer));
	  worth = -BIG_NUMBER;
	} else {
                                 ...
                                 struct player *aplayer)
{
  ...
  if (wants_ceasefire) {
    dai_diplo_notify(aplayer, _("*%s (AI)* Greetings %s! May we suggest a ceasefire "
                          ...
                          struct player *target, enum war_reason reason)
{
  ...
  case WAR_REASON_BEHAVIOUR:
    dai_diplo_notify(target, _("*%s (AI)* I have tolerated your vicious antics "
                               "long enough! To war!"),
                     player_name(pplayer));
    adip->countdown = -20;
    break;
  ...
  case WAR_REASON_HATRED:
    dai_diplo_notify(target, _("*%s (AI)* Finally I get around to you! Did "
                               "you really think you could get away with your crimes?"),
                     player_name(pplayer));
    adip->countdown = -20;
    break;
  case WAR_REASON_EXCUSE:
    dai_diplo_notify(target, _("*%s (AI)* Your covert hostilities brought "
                               "this war upon you!"),
                     player_name(pplayer));
    adip->countdown = -20;
    break;
  case WAR_REASON_ALLIANCE:
    if (adip->at_war_with_ally) {
      dai_diplo_notify(target, _("*%s (AI)* Your aggression against %s was "
                                 "your last mistake!"),
                       player_name(pplayer),
