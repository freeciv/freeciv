                   ...
                   const struct city *homecity)
{
  ...
  case ACTION_SPY_STEAL_GOLD_ESC:
     ...
     * gold target_player have. Not requiring it is therefore pointless.
     */
...
action_enabler_suggest_improvement(const struct action_enabler *enabler)
{
  ...
  if (action_get_actor_kind(paction) == AAK_UNIT) {
    ...
    unit_type_iterate(pactor) {
      if (action_actor_utype_hard_reqs_ok(paction->id, pactor)
                                        ...
                                        const struct tile *tgt_tile)
{
  ...
  case ATK_TILE:
    if (tgt_tile) {
      prob = action_prob_vs_tile(act_unit, paction->id, tgt_tile);
    }
    break;
  case ATK_CITY:
    if (tgt_city) {
