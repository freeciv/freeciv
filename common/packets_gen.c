...
static struct packet_city_info *receive_packet_city_info_100(struct connection *pc, enum packet_type type)
{
  ...
  real_packet->is_building_unit = BV_ISSET(fields, 25);
  if (BV_ISSET(fields, 26)) {
    {
      int readin;
  ...
  real_packet->changed_from_is_unit = BV_ISSET(fields, 28);
  if (BV_ISSET(fields, 29)) {
    {
      int readin;
  ...
  real_packet->airlift = BV_ISSET(fields, 39);
...
static int send_packet_city_info_100(struct connection *pc, const struct packet_city_info *packet)
{
  ...
  if(differ) {different++;}
  if(packet->is_building_unit) {BV_SET(fields, 25);}
  ...
  if(differ) {different++;}
  if(packet->changed_from_is_unit) {BV_SET(fields, 28);}
  ...
  if(differ) {different++;}
  if(packet->did_buy) {BV_SET(fields, 36);}

  differ = (old->did_sell != real_packet->did_sell);
  if(differ) {different++;}
  if(packet->did_sell) {BV_SET(fields, 37);}

  differ = (old->was_happy != real_packet->was_happy);
  if(differ) {different++;}
  if(packet->was_happy) {BV_SET(fields, 38);}

  differ = (old->airlift != real_packet->airlift);
  if(differ) {different++;}
  if(packet->airlift) {BV_SET(fields, 39);}

  differ = (old->diplomat_investigate != real_packet->diplomat_investigate);
  if(differ) {different++;}
  if(packet->diplomat_investigate) {BV_SET(fields, 40);}
...
static struct packet_unit_info *receive_packet_unit_info_100(struct connection *pc, enum packet_type type)
{
  ...
  real_packet->has_orders = BV_ISSET(fields, 24);
  if (BV_ISSET(fields, 25)) {
    {
      int readin;
  ...
  real_packet->orders_repeat = BV_ISSET(fields, 27);
  real_packet->orders_vigilant = BV_ISSET(fields, 28);
  if (BV_ISSET(fields, 29)) {
...
static int send_packet_unit_info_100(struct connection *pc, const struct packet_unit_info *packet)
{
  ...
  if(differ) {different++;}
  if(packet->has_orders) {BV_SET(fields, 24);}
  ...
  if(differ) {different++;}
  if(packet->orders_repeat) {BV_SET(fields, 27);}

  differ = (old->orders_vigilant != real_packet->orders_vigilant);
  if(differ) {different++;}
  if(packet->orders_vigilant) {BV_SET(fields, 28);}
...
BV_DEFINE(packet_ruleset_unit_100_fields, 34);
