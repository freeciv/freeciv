  ...
  if( land_cnt >= 3 ) {           /* enough land, scatter guys around */
    unit_list_iterate(map_get_tile(x, y)->units, punit2) {
      if( punit2->owner == me ) {
        ...
        } while( !handle_unit_move_request(barbarians, punit2, xu, yu) );
    ...
    if( sea_cnt > 0 ) {         /* maybe it's an island, try to get on boats */
      ...
      unit_list_iterate(map_get_tile(x, y)->units, punit2)
	    ...
	    if( can_unit_move_to_tile(punit2, xu, yu) ) break;
	    if( can_unit_move_to_tile(punit2, xb, yb) ) {
              xu = xb; yu = yb;
              break;
	    }
