  ...
  if (info->ver_units < 10) {
    unit_type_iterate(ptype) {
      ...
      if (utype_can_do_action(ptype, ACTION_SPY_INVESTIGATE_CITY)
          ...
          || utype_can_do_action(ptype, ACTION_SPY_STEAL_TECH_ESC)
          || utype_can_do_action(ptype, ACTION_SPY_TARGETED_STEAL_TECH)
          || utype_can_do_action(ptype, ACTION_SPY_TARGETED_STEAL_TECH_ESC)
  ...
  if (info->ver_game < 10) {
    ...
    action_enablers_iterate(ae) {
      ...
      if (ae->action == ACTION_SPY_STEAL_TECH) {
        ...
        /* The other allows non spies to do "Steal Tech". */
        requirement_vector_append(&enabler->actor_reqs,
                                  req_from_values(VUT_UTFLAG,
                                                  REQ_RANGE_LOCAL,
                                                  FALSE, FALSE, TRUE,
      ...
      if (ae->action == ACTION_SPY_TARGETED_STEAL_TECH) {
        ...
        /* The other allows non spies to do "Targeted Steal Tech". */
        requirement_vector_append(&enabler->actor_reqs,
                                  req_from_values(VUT_UTFLAG,
                                                  REQ_RANGE_LOCAL,
                                                  FALSE, FALSE, TRUE,
