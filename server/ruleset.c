    ...
    terrain_type_iterate(pterrain) {
      ...
      if (!lookup_time(file, &pterrain->plant_time,
        ...
        if (compat->compat_mode) {
          ...
          ruleset_error(NULL, LOG_ERROR, "%s: Missing plant_time", tsection);
          ok = FALSE;
          break;
        }
  ...
  if (ok) {
    ...
    nations_iterate(pnation) {
      ...
      for (j = 0; j < MAX_NUM_LEADERS; j++) {
        ...
        if (NULL == sex) {
          ruleset_error(NULL, LOG_ERROR, "Nation %s: leader \"%s\": %s.",
                                 ...
                                 struct rscompat_info *compat)
{
  ...
  section_list_iterate(sec, psection) {
    ...
    if (type == NULL) {
      ruleset_error(NULL, LOG_ERROR,
                    "\"%s\" [%s] missing effect type.", filename, sec_name);
      ok = FALSE;
      break;
    }
                              ...
                              struct rscompat_info *compat)
{
  ...
  if (ok) {
    ...
    if (ok) {
      ...
      if (reqs == NULL) {
        ...
        if (ok && compat->compat_mode) {
          ...
          if (ok && !rscompat_auto_attack_3_1(compat, auto_perf,
                                              psize, protecor_flag)) {
            /* Upgrade failed */
            ruleset_error(NULL, LOG_ERROR,
    ...
    if (ok) {
      ...
      if (ok && !lookup_bv_actions(file, filename,
      ...
      if (ok) {
         ...
         * expectations. */
        if (ok && secfile_entry_by_path(file, "actions.quiet_actions")) {
