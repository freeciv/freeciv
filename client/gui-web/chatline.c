...
struct astring messages = ASTRING_INIT;
...
{
  astr_add_line(&messages, "%s<br>", astring);
  /* FIXME: add player name from conn_id ? */
}

/**************************************************************************
 ...
**************************************************************************/
const char *get_output_window_text(void) 
{

  const char *result =  mystrdup(messages.str);
  astr_clear(&messages);
  return result;
} 
