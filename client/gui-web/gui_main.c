...
const char *client_string = "gui-web";
...
void ui_main(int argc, char *argv[])
{
  ...
  map_canvas_resized (1024, 650); /* FIXME*/
  ...
  while (i < argc) {
    if (is_option("--autostartserver", argv[i])) {
  ...
  if (auto_start_server) {
    freelog(LOG_NORMAL, "Freeciv web client starting server.");
    client_start_server();

  } else {

    freelog(LOG_NORMAL, "Freeciv web client connecting to a server.");
    start_autoconnecting_to_server();
    try_to_autoconnect();
  }

  /* Init the webservice (csoap API).  */
  err = soap_server_init_args(argc, argv);
  if (err != H_OK)
  {
    freelog(LOG_ERROR, "%s():%s [%d]", herror_func(err), herror_message(err),
               herror_code(err));
    herror_release(err);
  }

  /* FIXME: Look into alternative to using threads here.
   * It seems that the webservice and freeciv needs to 
   * run in different threads, tough.. */

  pthread_mutex_init(&civmutex, NULL);

  if ( pthread_create(&t1, NULL, (void *)init_webservice, (void *)"1") != 0 ) {
    freelog(LOG_ERROR, "pthread_create() error");
  }
