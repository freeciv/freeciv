...
void help_widget::do_layout()
{
  ...
  if (bottom_panel) {
    right = new QWidget();
    vbox = new QVBoxLayout(right);
    vbox->setMargin(0);
    vbox->addWidget(text_browser, 100);
    vbox->addWidget(bottom_panel);
//     vbox->addStretch(100);
  } else {
    right = text_browser;
  }
                                      ...
                                      const char *title)
{
  ...
  if (pterrain) {
    ...
    if (pterrain->mining_result == pterrain) {
      add_info_label(
        // TRANS: When mined, terrain gets a bonus of %1 shields;
        //        mining takes %2 turns
        QString(_(ngettext(
